...
from src.v9_new_se.fct import ki_67_percentage
...
if __name__ == "__main__":
    ...
    for idx_p, p_name in enumerate(p_names[0:MAX_PATIENTS]):
        ...
        for idx_img, (path_image, original_image) in enumerate(images(patient_name=p_name,
            ...
            regions_positive = outline_regions(resized_original[5:resized_original.shape[0] - 5,
                                               5:resized_original.shape[1] - 5], all_mask[0, :, :])
            io.imsave(fname=os.path.join(results_p_dir, f'regions positive.jpg'),
                      arr=regions_positive)
            regions_negative = outline_regions(resized_original[5:resized_original.shape[0] - 5,
                                               5:resized_original.shape[1] - 5], all_mask[1, :, :])
            io.imsave(fname=os.path.join(results_p_dir, f'regions negative.jpg'),
                      arr=regions_negative)
            regions_background = outline_regions(resized_original[5:resized_original.shape[0] - 5,
                                                 5:resized_original.shape[1] - 5], all_mask[2, :, :])
